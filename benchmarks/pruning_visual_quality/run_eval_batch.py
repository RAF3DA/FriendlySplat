...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "with extra columns for #Gaussians and training time (pruning benchmark outputs, 360v2)."
        ),
    )
    parser.add_argument(
        "--data-root",
    ...
    out_dir = data_root / "benchmark" / "pruning_benchmark"
    out_dir.mkdir(parents=True, exist_ok=True)
    md_path = out_dir / ("summary_all.md" if pruners_raw == "all" else "summary.md")
    ...
    for scene in scenes:
        ...
        for pruner in pruners:
            scene_out_dir = out_dir / scene / str(pruner)
            if not scene_out_dir.exists() and str(pruner) == "pure_densify":
                legacy = out_dir / scene / "dense"
                if legacy.exists():
                    scene_out_dir = legacy
            if not scene_out_dir.exists():
                print(
                    f"[skip] missing outputs: {dataset_key}/{scene} (pruner={pruner})",
                    flush=True,
                )
                continue
            ...
            try:
                ...
                print(
                    f"[ok] {dataset_key}/{scene} (pruner={pruner}) {summary}",
                    flush=True,
                )
                ...
                any_failed = True
                print(
                    f"[fail] {dataset_key}/{scene} (pruner={pruner}): {e}",
                    flush=True,
                )
