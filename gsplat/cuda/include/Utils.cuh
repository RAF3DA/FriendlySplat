...
posW2C(const mat3 R, const vec3 t,
...
inline __device__ mat3 quat_to_rotmat(const vec4 quat) {
  ...
  return mat3((1.f - 2.f * (y2 + z2)), (2.f * (xy + wz)),
              (2.f * (xz - wy)), // 1st col
...
inline __device__ void quat_to_rotmat_vjp(const vec4 quat, const mat3 v_R,
                                          vec4 &v_quat) {
  float w = quat[0], x = quat[1], y = quat[2], z = quat[3];
  // normalize
  float inv_norm = rsqrt(x * x + y * y + z * z + w * w);
...
inline __device__ void quat_scale_to_covar_preci(const vec4 quat,
                                                 const vec3 scale,
                                                 // optional outputs
                                                 mat3 *covar, mat3 *preci) {
  ...
  if (preci != nullptr) {
    // P = R * S^-1 * S^-1 * Rt
    mat3 S = mat3(1.0f / scale[0], 0.f, 0.f, 0.f, 1.0f / scale[1], 0.f, 0.f,
                  0.f, 1.0f / scale[2]);
    mat3 M = R * S;
...
inline __device__ void quat_scale_to_covar_vjp(
    // fwd inputs
    const vec4 quat, const vec3 scale,
    // precompute
    const mat3 R,
    // grad outputs
    const mat3 v_covar,
    // grad inputs
    vec4 &v_quat, vec3 &v_scale) {
  float w = quat[0], x = quat[1], y = quat[2], z = quat[3];
...
inline __device__ void quat_scale_to_covar_preci_half(const vec4 quat,
                                                      const vec3 scale,
                                                      // optional outputs
                                                      mat3 *covar_half,
                                                      mat3 *preci_half) {
...
inline __device__ void inverse_vjp(const mat2 Minv, const mat2 v_Minv,
                                   mat2 &v_M) {
  // P = M^-1
  // df/dM = -P * df/dP * P
...
inline __device__ float add_blur(const float eps2d, mat2 &covar,
                                 float &compensation) {
...
inline __device__ void add_blur_vjp(const float eps2d, const mat2 conic_blur,
                                    const float compensation,
                                    const float v_compensation, mat2 &v_covar) {
...
inline __device__ void ortho_proj(
    // inputs
    const vec3 mean3d, const mat3 cov3d, const float fx, const float fy,
...
inline __device__ void fisheye_proj(
  ...
  mat3x2 J = mat3x2(fx * (x2 * a + y2 * b), fy * xy * (a - b),
                    fx * xy * (a - b), fy * (y2 * a + x2 * b),
                    -fx * x * x2y2z2_inv, -fy * y * x2y2z2_inv);
  cov2d = J * cov3d * glm::transpose(J);
}
...
inline __device__ void fisheye_proj_vjp(
  ...
  // mat3x2 is 3 columns x 2 rows.
  mat3x2 J = mat3x2(fx * (x2 * J_a + y2 * J_b),
