...
struct RollingShutterParameters {
  ...
  __device__ RollingShutterParameters(const float *se3_start,
                                      const float *se3_end) {
    // input is row-major, but glm is column-major
    q_start = glm::quat_cast(glm::mat3(
        se3_start[0], se3_start[4], se3_start[8], se3_start[1], se3_start[5],
        se3_start[9], se3_start[2], se3_start[6], se3_start[10]));
    ...
    if (se3_end == nullptr) {
      ...
      q_end = glm::quat_cast(glm::mat3(se3_end[0], se3_end[4], se3_end[8],
...
eval_poly_odd_horner(std::array<float, N_COEFFS> const &poly_odd, float x) {
  ...
  return x * eval_poly_horner(poly_odd,
                              x * x); // evaluate x^2-based "regular" polynomial
...
template <size_t N_NEWTON_ITERATIONS, class PolyProxy, class DPolyProxy,
          class TInvPolyApproxProxy>
inline __host__ __device__ float
eval_poly_inverse_horner_newton(PolyProxy const &poly, DPolyProxy const &dpoly,
                                TInvPolyApproxProxy const &inv_poly_approx,
                                float y, bool &converged) {
  ...
  static_assert(N_NEWTON_ITERATIONS >= 0,
                "Require at least a single Newton iteration");
...
image_point_in_image_bounds_margin(glm::vec2 const &image_point,
...
template <class DerivedCameraModel> struct BaseCameraModel {
  ...
  inline __device__ auto image_point_to_world_ray_shutter_pose(
      ...
      RollingShutterParameters const &rolling_shutter_parameters) const
      -> WorldRay {
...
struct PerfectPinholeCameraModel : BaseCameraModel<PerfectPinholeCameraModel> {
  ...
  inline __device__ auto camera_ray_to_image_point(glm::fvec3 const &cam_ray,
...
struct OpenCVPinholeCameraModel
  ...
  __device__
  OpenCVPinholeCameraModel(Parameters const &parameters,
  ...
  inline __device__ auto compute_residual_and_jacobian(float x, float y,
    ...
    // fx(x, y) = fy(x, y) = 0;
    float fx =
        d * x + 2 * p1 * x * y + p2 * (r + 2 * x * x) + s1 * r + s2 * r2 - xd;
    float fy =
        d * y + 2 * p2 * x * y + p1 * (r + 2 * y * y) + s3 * r + s4 * r2 - yd;
  ...
  inline __device__ glm::fvec2
  compute_undistortion_newton(glm::fvec2 const &image_point,
...
struct OpenCVFisheyeCameraModel
  ...
  __host__ __device__ OpenCVFisheyeCameraModel(Parameters const &parameters,
    ...
    if (k4 == 0) {
      ...
      std::array<float, 4> ddforward_poly_odd = {6 * k1, 20 * k2, 42 * k3,
                                                 72 * k4};
      ...
      max_angle = eval_poly_inverse_horner_newton<N_NEWTON_ITERATIONS>(
          ...
          PolynomialProxy<PolynomialType::EVEN, 1>{approx}, 0.f, converged);
    ...
    max_angle =
        min(max_angle, max(max_radius_pixels / parameters.focal_length[0],
    ...
    auto const max_normalized_dist =
        std::max(parameters.resolution[0] / 2.f / parameters.focal_length[0],
  ...
  inline __device__ auto camera_ray_to_image_point(glm::fvec3 const &cam_ray,
    ...
    auto const image_point =
        glm::fvec2{parameters.focal_length[0] * delta * cam_ray.x +
                       parameters.principal_point[0],
  ...
  image_point_to_camera_ray(glm::fvec2 image_point) const {
    ...
    auto const theta = eval_poly_inverse_horner_newton<N_NEWTON_ITERATIONS>(
        ...
        PolynomialProxy<PolynomialType::EVEN, 5>{dforward_poly_even},
        PolynomialProxy<PolynomialType::FULL, 2>{approx_backward_poly}, delta,
        converged);
...
public:
  ...
  __host__ __device__ FThetaCameraModel(Parameters const &parameters,
                                        float min_2d_norm = 1e-6f)
    ...
    if (dist.reference_poly ==
      ...
      dreference_poly = {1.f * dist.pixeldist_to_angle_poly.at(1),
      ...
      dreference_poly = {1.f * dist.angle_to_pixeldist_poly.at(1),
    ...
    // FThetaCameraModelParameters are defined such that the image coordinate
    // origin corresponds to the center of the first pixel. We therefore need to
  ...
  inline __device__ auto camera_ray_to_image_point(glm::fvec3 const &cam_ray,
    ...
    // Limit angles to max_angle to prevent projected points to leave valid cone
    // around max_angle. In particular for omnidirectional cameras, this
    // prevents points outside the FOV to be wrongly projected to
    // in-image-domain points because of badly constrained polynomials outside
    ...
    // Apply linear term A=[c,d;e,1] to f(theta)-weighted normalized 2d vectors,
    ...
    image_point = glm::fvec2{c * image_point.x + d * image_point.y,
  ...
  image_point_to_camera_ray(glm::fvec2 image_point) const {
    ...
    image_point -= glm::fvec2{parameters.principal_point[0],
                              parameters.principal_point[1]};
    auto const uv = glm::fvec2{image_point.x - d * image_point.y,
...
world_gaussian_to_image_gaussian_unscented_transform_shutter_pose(
  ...
  auto const sigma_points = world_gaussian_sigma_points(
      unscented_transform_parameters, gaussian_world_mean, gaussian_world_scale,
      gaussian_world_rot);
